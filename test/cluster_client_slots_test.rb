...
class TestClusterClientSlots < Test::Unit::TestCase
  ...
  def test_slot_class
    ...
    assert_equal true, slot.not_exists?(0)
    assert_equal false, slot.not_exists?(1)
    assert_equal false, slot.not_exists?(10)
    assert_equal true, slot.not_exists?(11)

    assert_equal nil, slot.find_node_key(0)
    assert_equal '127.0.0.1:7000', slot.find_node_key(1)
    assert_equal '127.0.0.1:7000', slot.find_node_key(10)
    assert_equal nil, slot.find_node_key(11)
  ...
  def test_slot_class_with_node_flags_and_replicas
    ...
    assert_equal nil, slot.find_node_key(0)
    assert_true %w[127.0.0.1:7000 127.0.0.1:7001].include?(slot.find_node_key(1))
    assert_true %w[127.0.0.1:7000 127.0.0.1:7001].include?(slot.find_node_key(10))
    assert_equal nil, slot.find_node_key(11)
  ...
  def test_slot_class_with_empty_slots
    ...
    assert_equal nil, slot.find_node_key(0)
    assert_equal nil, slot.find_node_key(1)
