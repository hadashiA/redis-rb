...
require_relative 'cluster/key_extractor'
...
class Redis
  ...
  class Cluster
    ...
    def connection_info
      keys = %i[host port db id location]
      info = @node.map do |client|
        keys.map { |k| [k, client.public_send(k)] }.to_h
      end
      info.sort_by { |v| v[:id] }
    end
    ...
    def call_pipeline(pipeline)
      try_send(@node.sample, :call_pipeline, pipeline)
    end
    ...
    def send_command(command, &block)
      ...
      when 'exec'     then @node.call_master(command, &block).reject(&:empty?)
      when 'discard', 'multi', 'unwatch'
        @node.call_master(command, &block).first
      else
        node = assign_node(command)
    ...
    def try_send(node, method_name, *args, ttl: RETRY_COUNT, &block)
      ttl -= 1
      node.public_send(method_name, *args, &block)
    rescue CannotConnectError, ConnectionError => err
      raise if ttl <= 0
    ...
    def assign_node(command)
      ...
      return @node.sample if key.empty?
    ...
    def find_node(slot)
      return @node.sample if @slot.not_exists?(slot)

      node_key = @slot.find_node_key(slot)
      @node.find_by(node_key)
    rescue Node::ReloadNeeded
      update_cluster_info!(node_key)
      @node.find_by(node_key)
